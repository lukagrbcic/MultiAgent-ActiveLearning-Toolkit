...
initial_query = """This a code I use in my sampling loop with Random Forests. It is a greedy batched active learning sampling approach.
            ...
            GOALS:
            ------
            - Implement a single, **innovative batched active learning acquisition strategy** AND/OR introduce a substantially improved regression model to work with that strategy.
            ...
            - Improve uncertainty estimation:
                - Ensembles of models (bagging, stacking, voting)
                - Bayesian methods or MC Dropout
                - Multioutput-aware uncertainty aggregation
            ...
            ---------------------------------------------------
            - **Different regression models**: Replace Random Forest with:
                - Gradient Boosted Trees (e.g., XGBoost, LightGBM)
                - Gaussian Process Regression (possibly per output dimension or on reduced space)
                - Neural networks with dropout for uncertainty estimation
                - Ensemble of diverse regressors
            ...
            1. Produce **one complete Python class** `modelSampler` that:
                - Implements your chosen innovative batched acquisition strategy, OR
                - Replaces/improves the regression model for better high-dimensional multioutput performance, OR
                - Combines both in a unified approach.
            2. The acquisition/model combination must integrate seamlessly with the existing PSO optimizer for candidate point selection.
...
run_llm = llm_discovery(initial_query, 10, 0.79886)
result = run_llm.loop()
